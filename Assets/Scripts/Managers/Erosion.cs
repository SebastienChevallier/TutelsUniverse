...
public class Erosion : MonoBehaviour
{
    ...
    public void Erode(/*float[] map,*/ int mapSize, int numIterations = 1, bool resetSeed = false)
    {
        ...
        for (int iteration = 0; iteration < numIterations; iteration++)
        {
            ...
            float sediment = 0;
            float map = terrainData.GetHeight((int)posX, (int)posY);
            ...
            for (int lifetime = 0; lifetime < maxDropletLifetime; lifetime++)
            {
                ...
                if (sediment > sedimentCapacity || deltaHeight > 0)
                {
                    ...
                    map += amountToDeposit * (1 - cellOffsetX) * (1 - cellOffsetY);
                    map += amountToDeposit * cellOffsetX * (1 - cellOffsetY);
                    map += amountToDeposit * (1 - cellOffsetX) * cellOffsetY;
                    map += amountToDeposit * cellOffsetX * cellOffsetY;
                ...
                else
                {
                    ...
                    for (int brushPointIndex = 0; brushPointIndex < erosionBrushIndices[dropletIndex].Length; brushPointIndex++)
                    {
                        ...
                        float deltaSediment = (map < weighedErodeAmount) ? map : weighedErodeAmount;
                        map -= deltaSediment;
    ...
    HeightAndGradient CalculateHeightAndGradient(float nodes, int mapSize, float posX, float posY)
    {
        ...
        float heightNW = nodes;
        float heightNE = nodes;
        float heightSW = nodes;
        float heightSE = nodes;
